...
export function Car({ thirdPerson }: { thirdPerson: boolean }) {
  ...
  let gltf = useLoader(
    GLTFLoader,
    "/games/drift-city/models/car.glb"
  );
  let result = gltf.scene;

  const position = [-1.5, 0.5, 3] as [number, number, number];
  const width = 0.15;
  const height = 0.07;
  const front = 0.15;
  const wheelRadius = 0.05;

  const chassisBodyArgs = [width, height, front * 2] as [number, number, number];
  ...
  useFrame((state) => {
    ...
    position.setFromMatrixPosition((chassisBody.current as any).matrixWorld);

    let quaternion = new Quaternion(0, 0, 0, 0);
    quaternion.setFromRotationMatrix((chassisBody.current as any).matrixWorld);
